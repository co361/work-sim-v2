...
(function () {
  ...
  function renderTokens() {
    ...
    body.innerHTML = rows.map(function (r) {
      ...
      return '<tr>' +
        ...
        '<td>' + esc(r.user || '—') + '</td>' +
        '<td class="num">' + (r.day ? (+r.day||0) + '일차' : (r.err ? '<span class="pill p-revoked" title="' + esc(r.err) + '">조회 실패</span>' : '—')) + '</td>' +
        '<td>' + esc(r.memo || '—') + '</td>' +
  ...
  function renderProgress() {
    ...
    body.innerHTML = rows.map(function (r) {
      return '<tr class="pick" data-code="' + esc(r.code) + '" tabindex="0">' +
        ...
        '<td>' + esc(TEAM_NAME[r.team] || r.team || '—') + '</td>' +
        '<td>' + esc(r.name || '—') + '</td>' +
        '<td class="num">' + (+r.day||0) + '일차</td>' +
        '<td class="num">' + (r.lastDone ? (+r.lastDone||0) + '일' : '—') + '</td>' +
        '<td class="num">' + (r.avg == null ? '—' : (+r.avg||0)) + '</td>' +
  ...
  function studentHtml(code, p) {
    ...
    var out = '<div class="kv">' +
      ...
      '<b>완료한 날</b><span>' + (s.days.length ? s.days.map(function(d){return +d||0;}).join(', ') + '일차' : '없음') + '</span>' +
      '<b>평균 점수</b><span>' + (s.avg == null ? '—' : (+s.avg||0) + '점') + '</span>' +
