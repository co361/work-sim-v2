...
function openComposer(id,which,key){ const c=CARD(id); which=which||'reply'; S.composing={id,which,key:key||'reply'}; const box=$('composer'); box.style.display='block';
  ...
  const needWork=!!(c.workAnswer||c.hasWork)&&!(flowOf(c).includes('work')); const wa=$('cpWork'); wa.style.display=needWork?'flex':'none'; $('cpWorkIn').value=''; if(needWork) $('cpWorkIn').placeholder=c.workHint||'결과값 (예: 39일, 62,000원)';
  const meta=specMeta(c,which==='reply2'?'second':null); $('cActs').innerHTML='';
  const tips=[]; if(meta.must) tips.push(`반드시 들어갈 값 ${meta.must}가지`); if(meta.rules&&(D.unlock||[]).includes('rulebook')) tips.push('근거 조항 번호를 붙여요(오른쪽 사규집)'); if(!tips.length) tips.push('인사 · 확인 · 답 · 다음 행동 · 맺음, 다섯 가지를 담아 보세요.');
...
async function sendCompose(id,text,which,key,ans){ const c=CARD(id), st=S.cards[id]; which=which||'reply'; key=key||'reply'; closeComposer(); $('composeSend').disabled=true;
  try{
    ...
    /* 무채점(동기 메신저·기록용) */
    if(r.unscored){ st.source=null; if(c.clue&&c.clue.requiresReply) captureClue(c,st,true); markStep(id,which,{text}); for(const k of r.branches) runBranch(id,k); return; }
    applyFlags(st,r.flags); applyCounts(r.counts); if(r.bestComment) st.bestComment=r.bestComment; if(r.model) st.model=r.model;
    if(r.retry){ toast('결과값이 맞지 않아요. 다시 세어 보고 보내 주세요.'); $('composeSend').disabled=false; openComposer(id,which,key); $('composeText').value=text; $('cpWorkIn').value=ans; return; }
...
async function sendApproval(id,text,ans){ const c=CARD(id), st=S.cards[id]; st.text=text;
  ...
  applyFlags(st,r.flags); applyCounts(r.counts); if(r.bestComment) st.bestComment=r.bestComment; if(r.model) st.model=r.model; for(const k of r.branches) runBranch(id,k);
...
async function doApprove(id){ const c=CARD(id); let r; try{ r=await Grader.run('approve',c,{}); }catch(e){ return gradeFail(id,e); } S.counts.rejectAll++; if(r.bestComment) S.cards[id].bestComment=r.bestComment; markStep(id,'approval',{score:r.score,comment:r.comment,key:'approve'}); if(r.branch) runBranch(id,r.branch); }
...
async function doButton(id,k,branchKey){ const c=CARD(id); const st=S.cards[id]; if(!(c.act&&c.act[k])||st.status==='done') return; let r; try{ r=await Grader.run('act',c,{key:k}); }catch(e){ return gradeFail(id,e); } if(!r.ok||st.status==='done') return;
  ...
  st.choice=k; if(r.bestComment) st.bestComment=r.bestComment; record(id,{act:k,score:r.score,comment:r.comment}); if(!r.isBest){ runBranch(id,branchKey||k)||runBranch(id,'other'); } else runBranch(id,branchKey||'ok'); }
...
async function doMailConfirm(id){ const c=CARD(id), st=S.cards[id]; if(!c||!st||st.status==='done') return;
  ...
  if(r.mode==='act'){ /* 분기 표에 「메일로 상신」 행이 없는 카드는 act.confirm 이 데이터 점수다 */
    if(!r.ok) return; S.counts.ask++; if(r.bestRC){ S.counts.rejectAll++; if(r.isBest) S.counts.reject++; } st.choice='confirm'; if(r.bestComment) st.bestComment=r.bestComment; record(id,{act:'confirm',score:r.score,comment:r.comment}); if(!r.isBest){ runBranch(id,'mailConfirm')||runBranch(id,'other'); } else runBranch(id,'mailConfirm'); return; }
  st.lastNpc=c.npc||st.lastNpc; st.lastSeat='lead';
  if(r.mode==='line'){ st.mailTried=true; runBranch(id,'mailConfirm'); if(S.cur===id) renderCardActs(id); renderInbox(); saveProgress(); return; }
  S.counts.ask++; if(r.bestRC) S.counts.rejectAll++;
  st.choice='mailConfirm'; st.reportBest=false;
  record(id,{act:'confirm',score:r.score,comment:'메일로만 올렸어요. 이 건은 근거를 들고 팀장 자리로 가서 직접 보고해야 해요.'}); runBranch(id,'mailConfirm'); }
async function doPhone(id,k,keepAfter){ const c=CARD(id); const st=S.cards[id]; if(!(c.act&&c.act[k])||st.status==='done') return null; let r; try{ r=await Grader.run('act',c,{key:k}); }catch(e){ gradeFail(id,e); return null; } if(!r.ok||st.status==='done') return null;
  st.choice=k; if(k==='promise') S.counts.promise++; if(r.bestComment) st.bestComment=r.bestComment; record(id,{act:k,score:r.score,comment:r.comment});
  ...
  return after||null; }
async function doPick(id,pick){ const c=CARD(id); const pc=CARD(pick); let r; try{ r=await Grader.run('pick',c,{pick}); }catch(e){ return gradeFail(id,e); } S.cards[id].text=`"${pc.subj}" 건이요.`; record(id,{act:'reply',score:r.ok?100:60,comment:r.comment||'',text:S.cards[id].text}); runBranch(id,r.ok?'ok':'fallback'); }
async function doWork(id,ans){ const c=CARD(id), st=S.cards[id]; if(st.status==='done'||(st.steps&&st.steps.work)) return; let w; try{ w=await Grader.run('work',c,{ans}); }catch(e){ return gradeFail(id,e); } const shown=workText(c,ans); st.answer=shown; S.counts.calcAll++; if(w.bestComment) st.bestComment=w.bestComment;
...
async function doReport(id,auto){ const c=CARD(id), st=S.cards[id]; const O=office(); if(O&&(O.busy||O.walking)){ toast('지금은 이동할 수 없어요'); return; } const rp=c.report||c; const keys=Object.keys(rp.choices||{}); const labels=keys.map(k=>rp.choices[k]); const best=keys.indexOf(c.best in (rp.score||{})?c.best:(rp.best||keys[0]));
  ...
  const k=keys[r.choice]; let sc; try{ sc=await Grader.run('report',c,{key:k}); }catch(e){ return gradeFail(id,e); } const score=sc.score; st.choice=k; st.reportChoice=k; st.lastNpc=who; st.lastSeat='lead';   /* 분기 반응의 「@npc」가 「상대」로 뜨지 않게 */ const isBest=!!sc.isBest; if(sc.bestComment) st.bestComment=sc.bestComment;
...
async function doVisit(id,auto){ const c=CARD(id), st=S.cards[id]; const keys=c.choiceOrder||Object.keys(c.choices||{}); const labels=keys.map(k=>c.choices[k]); const best=keys.indexOf(c.best); const open=(c.body.match(/"([^"]+)"/)||[])[1]||c.subj;
  ...
  let a; try{ a=await Grader.run('act',c,{key:k}); }catch(e){ return gradeFail(id,e); } if(!a.ok) a={score:30,comment:'',branch:k}; if(st.status==='done') return; st.choice=k; if(k==='refund') S.counts.promise++; if(a.bestComment) st.bestComment=a.bestComment; record(id,{act:k,score:a.score,comment:a.comment});
...
async function doSheetDeliver(id,auto){ const c=CARD(id), st=S.cards[id]; const d=c.deliver; const keys=Object.keys(d.choices||{}); const labels=keys.map(k=>d.choices[k]); const bestK=Object.entries(d.score||{}).sort((a,b)=>b[1]-a[1])[0]; const best=bestK?keys.indexOf(bestK[0]):0; const who=d.npc||'팀장';
  ...
  const k=keys[r.choice]; let sc; try{ sc=await Grader.run('sheetDeliver',c,{key:k}); }catch(e){ return gradeFail(id,e); } const score=sc.score; st.choice=k; st.lastNpc=who; st.lastSeat=seat; const isBest=!!sc.isBest; if(sc.bestComment) st.bestComment=sc.bestComment; if(isBest){ S.trust[who]=(S.trust[who]||0)+1; S.counts.rightNpc++; } runBranch(id,isBest?'ok':k,{toastOnly:true});
...
function msgActions(id,foot){ const c=CARD(id), st=S.cards[id];
  ...
  if(flow.includes(which)&&!steps[which]&&isTextCard(c)&&!(flow.includes('deliver')&&!steps.deliver)){
    ...
    if(meta.must) t.push(`반드시 들어갈 값 ${meta.must}가지`);
    if(meta.rules&&(D.unlock||[]).includes('rulebook')) t.push('근거 조항 번호를 붙여요');
...
function finalize(id){ const c=CARD(id), st=S.cards[id]; const steps=st.steps||{}; const parts=[]; let comment=[]; let act='reply';
  ...
  { const bc=(c.act&&c.act[c.best])?(c.act[c.best][1]||''):(st.bestComment||''); if(!comment.length&&score!=null&&(c.act&&c.act[c.best]||st.bestComment)&&(score>=80)) comment.push(bc); }
