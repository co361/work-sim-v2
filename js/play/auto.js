...
(function(){
  ...
  async function playCard(c,opt){ const id=c.id; const st=S.cards[id]; const flow=flowOf(c); openCard(id); const fail=opt.fail&&!opt.failed;
    if(c.followup){ await doButton(id,'confirm'); return; }
    if(c.recordLookup&&!st.lookup){ st.lookup=true; }
    if(c.actions&&c.actions.stopShip) st.stopShip=true;
    if(c.mode==='phone'){ const keys=c.choiceOrder||Object.keys(c.choices); let k=c.best; if(fail){ k=keys.find(x=>x!==c.best&&c.act[x]&&c.act[x][0]<=20)||keys.find(x=>x!==c.best); opt.failed=id; } await doPhone(id,k); return; }
    if(c.mode==='visit'){ const keys=c.choiceOrder||Object.keys(c.choices); await pump(()=>doVisit(id,keys.indexOf(c.best))); return; }
    if(c.mode==='reflect'){ await doPick(id,(c.answerAny||[])[0]||S.order[0]); return; }
    if(flow.includes('work')){ const cells=workCells(c); await doWork(id,cells?Object.fromEntries(cells.map(x=>[x.key,String(x.answer)])):c.workAnswer); if(c.deliver&&S.cards[id].status!=='done'){ const keys=Object.keys(c.deliver.choices||{}); const bi=bestIndex(keys,c.deliver.score||{},null); await pump(()=>doSheetDeliver(id,bi)); } return; }
    if(flow.includes('approval')){ let text=modelFor(c); if(!c.compose) text='반려합니다. '+(c.mustFlag||[]).map(k=>[].concat(k)[0]).join(' · ')+' 기준이 규정과 다릅니다. 맞는 합계는 '+(c.workAnswer||'')+'입니다.'; await sendCompose(id,text,'approval','reject',c.workAnswer||null); return; }
    ...
    let key='reply'; if(c.best&&['reject','confirm'].includes(c.best)&&(hasCompose(c)||c.alsoReply)) key=c.best; else if(c.best&&c.act&&c.act[c.best]&&!['reply','reject','confirm'].includes(c.best)){ await doButton(id,c.best); return; }
    if(c.best==='confirm'&&!hasCompose(c)&&!c.alsoReply){ await doButton(id,'confirm'); return; }
    if(c.best==='reject'&&!hasCompose(c)){ await doButton(id,'reject'); return; }
  ...
  async function autoDay(opt){ if(S.phase==='briefing') endBriefing();
    if(S.phase==='triage'&&S.triage){ const T=D.triage; for(const id of S.triage.ids){ S.triage.assign[id]=(T.now||[]).includes(id)||!(T.cards||[]).includes(id)?'now':(T.morning||[]).includes(id)?'morning':'today'; } await finishTriage(); }
    let guard=0;
